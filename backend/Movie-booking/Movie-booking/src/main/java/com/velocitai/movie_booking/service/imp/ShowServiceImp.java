...
public class ShowServiceImp implements ShowService {
	...
	public ResponseEntity<Show> saveShow(Show show, long theaterId, long movieId) {
		...
		show.setTheater(theaterRepository.findById(theaterId).get());
	...
	public ResponseEntity<?> deleteShow(Long id) {
		...
		List<Ticket> tickets=	ticketRepository.findByShowTimeId(show.getId());
		System.out.println(tickets);
		if(!tickets.isEmpty()) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Show cannot be deleted because ticket is already booked");
		}
			else {	
		List<Seat> seats = show.getSeat();
			if (!seats.isEmpty()) {
